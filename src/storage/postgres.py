...
class StorageLayer:
    ...
    def save_task(self, task) -> str:
        ...
        """
        self.db.execute(sql, (
            task.task_id, task.session_id, task.parent_task_id, task.title,
            task.description, task.category, task.status.value,
            task.assignee_agent_id, task.retry_count, task.max_retries
        ))
        return task.task_id
    ...
    def update_task(self, task_id: str, status: str = None, result: Dict = None,
                   error_message: str = None, completed_at: datetime = None, retry_count: int = None):
        """Update task"""
    ...
    def _row_to_task(self, row) -> Dict:
        ...
        return {
            "task_id": row[0], "session_id": row[1], "parent_task_id": row[2],
            "title": row[3], "description": row[4], "category": row[5],
            "status": row[6], "assignee_agent_id": row[7],
            "result": row[8], "error_message": row[9],
            "retry_count": row[10], "max_retries": row[11],
            "created_at": row[12], "updated_at": row[13], "completed_at": row[14]
        }
    ...
    def update_session(self, session_id: str, final_output: str = None, 
    ...
    def get_session(self, session_id: str) -> Optional[Dict]:
        ...
        if row:
            return {
                "session_id": row[0], "user_input": row[1],
                "final_output": row[2], "summary": row[3],
                "status": row[4], "created_at": row[5], "completed_at": row[6]
            }
        return None
    
    # ========== Agent Contexts ==========
    def save_agent_context(self, session_id: str, agent_id: str, context_data: Dict) -> int:
        """Save agent context"""
        sql = """
    ...
    def get_agent_context(self, session_id: str, agent_id: str) -> Optional[Dict]:
        ...
        if row:
            return {
                ...
                "context_data": row[3], "updated_at": row[4]
            }
        return None
    ...
    # ========== Task Progress ==========
    def save_task_progress(self, task_id: str, agent_id: str, progress: float, message: str = ""):
        """Save task progress"""
        sql = """
    ...
    def get_task_progress_history(self, task_id: str) -> List[Dict]:
        ...
        for row in rows:
            results.append({
                "id": row[0], "task_id": row[1], "agent_id": row[2],
                "progress": row[3], "message": row[4], "created_at": row[5]
            })
        return results
