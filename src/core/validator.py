...
class BaseValidator:
    ...
    def _check_required_fields(self, result: Dict, required: List[str]) -> ValidationResult:
    ...
    def validate(self, result: Dict[str, Any], category: str) -> ValidationResult:
        ...
        for field in required:
            if field in result:
                ...
                elif field == "items" and len(result[field]) < self.MIN_ITEMS.get(category, 1):
                    vr.add_warning(field, f"Low recommendation count: {len(result[field])}")
...
class ResultValidator:
    ...
    def validate(self, result: Any, validator_name: str = "outfit", 
                category: str = None) -> ValidationResult:
        """
        Validate result
    ...
    def auto_fix(self, result: Any, validator_name: str = "outfit",
        ...
        if isinstance(result, str):
            try:
                ...
                return {"items": ["Fix failed"], "colors": [], "styles": [], "reasons": []}
