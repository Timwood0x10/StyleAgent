...
def display_result(profile: dict, result: dict, lang: str = "zh"):
    ...
    if lang == "zh":
        ...
        print(
            f"📝 心情: {profile['mood']} | 场合: {profile['occasion']} | 预算: {profile['budget']}"
        )
        print("=" * 50)
        ...
        print(
            f"📝 Mood: {profile['mood']} | Occasion: {profile['occasion']} | Budget: {profile['budget']}"
        )
        print("=" * 50)
...
def main():
    ...
    parser.add_argument(
        "-l", "--lang", choices=["zh", "en"], default="zh", help="输出语言"
    )
    ...
    if lang == "zh":
        ...
        print(
            f"✅ 已解析: {profile['name']}, {gender_str}, {profile['age']}岁, {profile['occupation']}"
        )
    else:
        print(
            f"✅ Parsed: {profile['name']}, {profile['gender']}, {profile['age']}, {profile['occupation']}"
        )
